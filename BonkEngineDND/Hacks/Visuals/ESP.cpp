...
FVector RotatePoint(FVector EntityPos, FVector LocalPlayerPos, float angle, bool* ViewCheck)
{
	...
	x1 = (float)(r2 * ((float)cos((double)YawToRadian)) - r1 * sin((double)(YawToRadian))) / 20;
	y1 = (float)(r2 * (float)sin((double)YawToRadian)) - r1 * cos((double)(YawToRadian)) / 20;
...
void RenderRadarActors(FVector baseActorLocation, AActor* targetActor, APlayerController* playerController, APlayerCameraManager* playerCameraManager)
{
	...
	RenderFilledRect({ RotationPoint.x, RotationPoint.y, 0.0f }, 3, 3, { 255, 0, 0, 255 });
}
...
void ESP()
{
	...
	for (auto levelIndex = 0; levelIndex < Levels.Num(); levelIndex++)
	{
		...
		for (int index = 0; index < level->Actors.Num(); index++)
		{
			...
			auto localLocaation = Controller->AcknowledgedPawn->K2_GetActorLocation();
			FVector2D radarResult;
			if (bRadar)
			{
				if (Actor->IsA(ADCPlayerCharacterBase::StaticClass()))
				{
					RenderRadarActors(localLocaation, Actor, Controller, Camera);
				}
			}
